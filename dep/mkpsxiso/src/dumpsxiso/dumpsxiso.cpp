...
int _dumpsxiso(int argc, char *argv[])
{
	param::isoFile = std::filesystem::path();
	param::outPath = std::filesystem::path();
	param::xmlFile = std::filesystem::path();
...
int dumpsxiso(int argc, wchar_t* argv[])
{
	...
	return _dumpsxiso(argc, u8argv.data());
}
#else
int dumpsxiso(int argc, char* argv[])
{
	return _dumpsxiso(argc, argv);
}
#endif